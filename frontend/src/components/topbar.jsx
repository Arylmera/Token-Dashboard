...
export const Topbar = ({ range, setRange, provider = "all", setProvider, themeId }) => {
  ...
  return (
  // `deep` = the whole subtree drags the undecorated window (bare
  // data-tauri-drag-region only reacts to direct clicks on the element itself,
  // which left most of the bar dead). Interactive clusters opt out with
  // `="false"`; buttons/inputs are excluded by Tauri's own handler.
  <header className="a-topbar" data-tauri-drag-region="deep">
